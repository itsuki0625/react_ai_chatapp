...
class HistoryStepAgent:
    def __init__(self):
        tools = [note_store, list_notes]
        self.agent = build_step_agent(HISTORY_PROMPT, tools)
    ...
    async def __call__(self, params: dict):
        ...
        try:
            ...
            logger.info(f"HISTORY agent raw response: {response}")
            ...
            logger.error(f"Error in HISTORY agent: {e}")
            # Return a fallback response
            return "申し訳ありませんが、過去分析中にエラーが発生しました。もう一度お試しください。" 