...
class GapStepAgent:
    def __init__(self):
        ...
        self.agent = build_step_agent(GAP_PROMPT, tools)
    ...
    async def __call__(self, params: dict):
        ...
        try:
            ...
            logger.info(f"GAP agent raw response: {response}")
            ...
            logger.error(f"Error in GAP agent: {e}")
            # Return a fallback response
            return "申し訳ありませんが、ギャップ分析中にエラーが発生しました。もう一度お試しください。" 