...
class MotivationStepAgent:
    def __init__(self):
        tools = [note_store, list_notes]
        self.agent = build_step_agent(MOTIVATION_PROMPT, tools)
    ...
    async def __call__(self, params: dict):
        ...
        try:
            ...
            logger.info(f"MOTIVATION agent raw response: {response}")
            ...
            logger.error(f"Error in MOTIVATION agent: {e}")
            # Return a fallback response
            return "申し訳ありませんが、動機分析中にエラーが発生しました。もう一度お試しください。" 